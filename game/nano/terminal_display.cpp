...
void print_frame(const std::array<std::array<int, MAX_X>, MAX_Y> &frame) {
  // Only print the first element of the 2D array (frame[0][0])
  int first_element = frame[0][0];

  // Print the first element to the serial monitor
  if (first_element == 1) {
    Serial.println("█");
  } else {
    Serial.println(".");
...
// void print_frame(const std::array<std::array<int, MAX_X>, MAX_Y> &frame) {
// 	// Note that the █ character takes two bytes to store, so the buffer
// length
// 	// needs to be big
